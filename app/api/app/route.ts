...
import { ensurePaymentSchema, handlePaymentAction, loadPaymentData } from "./payments";
...
export async function GET(request:Request){
  ...
  const paymentData=await loadPaymentData({communityId,role:membership.role,unit:membership.unit,userId:user.userId});
  ...
  const [community,membersResult,residentsResult,decisions,notifications,invitations]=await Promise.all([
...
export async function POST(request:Request){
  ...
  if(action==="addResident"){
    ...
    const dueDate=clean(body.dueDate,20),amount=number(body.amount,1,10_000_000),period=clean(body.period,7)||dueDate.slice(0,7),config=await db.prepare("SELECT late_interest_rate FROM communities WHERE id=?").bind(communityId).first<{late_interest_rate:number}>();if(!dueDate||!period)return fail("Dönem ve son ödeme tarihi gerekli.");const [manual,accounts,existing]=await Promise.all([db.prepare("SELECT name,unit FROM residents WHERE community_id=?").bind(communityId).all<{name:string;unit:string}>(),db.prepare("SELECT display_name AS name,unit FROM members WHERE community_id=? AND role='resident' AND unit IS NOT NULL").bind(communityId).all<{name:string;unit:string}>(),db.prepare("SELECT unit FROM dues WHERE community_id=? AND period=? AND kind='monthly'").bind(communityId,period).all<{unit:string}>()]);
    const people=[...manual.results,...accounts.results].filter((person,index,list)=>list.findIndex(item=>item.unit===person.unit)===index).filter(person=>!existing.results.some(item=>item.unit===person.unit));if(!people.length)return fail("Bu dönem için eklenecek yeni daire bulunamadı.",409);await db.batch(people.map(person=>db.prepare("INSERT INTO dues (id,community_id,resident_name,unit,amount,period,kind,interest_rate,status,due_date,created_by,created_at) VALUES (?,?,?,?,?,?,'monthly',?,'pending',?,?,?)").bind(id(),communityId,person.name,person.unit,amount,period,config?.late_interest_rate||0,dueDate,user.userId,createdAt)));return NextResponse.json({ok:true,created:people.length});
  }else if(action==="addDue"){const name=clean(body.residentName,100),unit=clean(body.unit,60),dueDate=clean(body.dueDate,20),period=clean(body.period,7)||dueDate.slice(0,7),kind=clean(body.kind,20)==="Ek borç"?"extra":"monthly",config=await db.prepare("SELECT late_interest_rate FROM communities WHERE id=?").bind(communityId).first<{late_interest_rate:number}>();if(!name||!unit||!dueDate)return fail("Sakin, daire ve son ödeme tarihi gerekli.");await db.prepare("INSERT INTO dues (id,community_id,resident_name,unit,amount,period,kind,note,interest_rate,status,due_date,created_by,created_at) VALUES (?,?,?,?,?,?,?,?,?,'pending',?,?,?)").bind(id(),communityId,name,unit,number(body.amount,1,10_000_000),period,kind,clean(body.note,500)||null,config?.late_interest_rate||0,dueDate,user.userId,createdAt).run();
  ...
  }else if(action==="updateCommunity"){const name=clean(body.name,100),paymentLink=clean(body.paymentLink,500);if(!name)return fail("Apartman adı gerekli.");if(paymentLink&&!/^https:\/\//i.test(paymentLink))return fail("Online ödeme bağlantısı https:// ile başlamalıdır.");await db.prepare("UPDATE communities SET name=?,block_count=?,unit_count=?,monthly_due=?,period=?,auto_due_enabled=?,due_day=?,late_interest_rate=?,payment_link=? WHERE id=?").bind(name,number(body.blockCount,1,50),number(body.unitCount,1,2000),number(body.monthlyDue,0,10_000_000),clean(body.period,30),body.autoDueEnabled==="on"?1:0,number(body.dueDay,1,28),number(body.lateInterestRate,0,100),paymentLink||null,communityId).run();
  ...
  }else if(action==="delete"){const tables:Record<string,string>={residents:"residents",dues:"dues",expenses:"expenses",incomes:"incomes",announcements:"announcements",decisions:"decisions"},table=tables[clean(body.type,30)];if(!table)return fail("Geçersiz kayıt türü.");await db.prepare(`DELETE FROM ${table} WHERE id=? AND community_id=?`).bind(clean(body.id,80),communityId).run();
